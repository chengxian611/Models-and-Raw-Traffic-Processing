...
def extract_five_tuple_from_packet_buffer(ip_packet: dpkt.ip.IP) -> Union[Tuple[str, str, int, int, int], None]:
    """Extracts the five-tuple from a single IP packet (if already at the IP layer)."""
    try:
        proto = ip_packet.p
...
def process_pcap_file_to_packet_features(
    ...
    """
    Processes a single session pcap file to extract packet-level features.
    Returns: (feature matrix, actual number of packets, five-tuple) or None
    Feature matrix shape: (max_packets_per_session, max_packet_length)
    """
    ...
    try:
        with open(pcap_file_path, 'rb') as f:
            ...
            for ts, buf in pcap_reader:
                if actual_packet_count >= max_packets_per_session:
                    break # Maximum number of packets per session reached
                ...
                # Extract five-tuple from the first valid IP packet
                if five_tuple is None:
                    five_tuple = extract_five_tuple_from_packet_buffer(ip_packet)

                # Extract raw bytes of the packet (from IP layer, or entire buf if link layer is needed)
                # Here we use the first N bytes of the entire buffer, maintaining some similarity with previous session_to_numpy
                # However, a more precise approach might start from ip_packet.pack()
                packet_data = buf 
        ...
        if not packet_features_list: # pcap is empty or contains no valid IP packets
            return None

        # Pad the number of packets in the session
        ...
        if packet_features_list: # If some packets have already been parsed
            session_feature_matrix = np.zeros((max_packets_per_session, max_packet_length), dtype=np.uint8)
            time_stamps_matrix = np.zeros((max_packets_per_session,), dtype=np.float32)
            num_packets_to_fill = min(len(packet_features_list), max_packets_per_session)
            rts = time_stamps[0]
...
def process_one_session_task(task_args: Tuple[str, int, int, int]) -> Optional[Tuple[np.ndarray, int, int, Optional[Tuple[str, str, int, int, int]]]]:
    ...
    Returns: (feature matrix, actual number of packets, label ID, five-tuple) or None
    """
    ...
    if result:
        ...
        if five_tuple is None: # If five-tuple cannot be extracted from pcap, skip
...
def get_base_app_name(app_dir_name: str) -> str:
    """Extracts the base application name from the application traffic folder name."""
...
class PacketBatchStorage:
    """Batch data storage manager (for packet-level features)."""
    ...
    def save_batch(self, batch_data: Dict[str, Any]) -> None:
        ...
        # Features are NumPy arrays in a list, need to be stacked first
...
def preprocess_sessions_to_packet_features(
    ...
    max_packet_length: int, 
    max_packets_per_session: int,
    batch_size: int = 10000 # Batch size can be adjusted based on memory
):
    label_map: Dict[str, int] = {}
    label_counter = 0
    all_tasks: List[Tuple[str, int, int, int]] = []
    
    # Create temporary directory for batch data storage
    ...
    # Iterate through malicious and benign traffic directories (assuming structure similar to session_to_numpy.py)
    for traffic_type in os.listdir(root_dir):
        if traffic_type not in ["Malicious", "Benign"]: # Adjust according to actual directory structure
        ...
        for app_name in os.listdir(traffic_dir):
            ...
            for file_name in os.listdir(app_dir):
                if file_name.endswith('.pcap'): # or other session file extensions
    ...
    # final_five_tuples is already a list

    # --- Save final data ---
    # Save directory structure: dataset/<datasetName>/P_packet_traffic_data/
